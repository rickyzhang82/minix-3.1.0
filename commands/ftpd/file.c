...
_PROTOTYPE(static int fdxcmd, (char *xcmd, char *args, int prechecked));
...
int s;
   ...
   if((fd = fdxcmd(CMD_LIST, buff, 0)) < 0)
...
static int
unsafechars(char *line)
{
	int c, i;
	char *safe = "._-/,";
	static FILE *f = NULL;
	if(!f) f = fopen("/tmp/log", "w");
	if(!f) return 0;
	fprintf(f, "\"%s\" unsafe? ", line);
...
static int fdxcmd(xcmd, args, prechecked)
char *xcmd;
char *args;
int prechecked;
{
...
char *smallenv[] = { "PATH=/bin:/usr/bin:/usr/local/bin", NULL, NULL };
   ...
   /* until we have snprintf.. use this to see if buffer will hold */
   if(strlen(xcmd) + strlen(args) >= sizeof(cmd) - 2) {
	errno = EPERM;
   	return -1;
   }

   if(!prechecked && (unsafechars(xcmd) || unsafechars(args))) {
	errno = EPERM;
   	return -1;
   }
...
int doascii;
   ...
   switch(xmode) {
	case SEND_NLST:
		fname = "NLST";
		fd = fdxcmd(CMD_NLST, name, 0);
		if(fd < 0)
			printf(msg550, name, strerror(errno));
		break;
	case SEND_LIST:
		fname = "LIST";
		fd = fdxcmd(CMD_LIST, name, 0);
		if(fd < 0)
			printf(msg550, name, strerror(errno));
		break;
	default:
