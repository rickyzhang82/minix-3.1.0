...
# define MINIXFS_V1    "minix"
# define MINIXFS_V2    "minix v2"
# define MINIXFS_V3    "minix v3"
# define UNKNOWNFS     "unknown"

  extern char *program_name; /* All coreutils have this. */

  {
    ...
    if (load_mtab(program_name) < 0) return NULL;

    while (get_mtab_entry(special, mounted_on, version, rw_flag) == 0) {
	me = xmalloc(sizeof *me);
        me->me_devname = xstrdup(special);
        me->me_mountdir = xstrdup(mounted_on);

        /* Regardless of need_fs_type we always fill in me_type. */
        if (version[0] == '1' && version[1] == '\0') {
            me->me_type = MINIXFS_V1;
        } else if (version[0] == '2' && version[1] == '\0') {
            me->me_type = MINIXFS_V2;
        } else if (version[0] == '3' && version[1] == '\0') {
            me->me_type = MINIXFS_V3;
        } else {
            me->me_type = UNKNOWNFS;
        }

        /* Fetch major/minor device number of me_mountdir. */
        saved_errno = errno;
        ...
        /* Common for all filesystems known to Minix (at this time). */
        me->me_dummy = ME_DUMMY (me->me_devname, me->me_type);
        me->me_remote = ME_REMOTE (me->me_devname, me->me_type);
        me->me_type_malloced = 0;
