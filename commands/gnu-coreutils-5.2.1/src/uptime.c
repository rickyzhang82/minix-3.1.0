...
/*printf("print_uptime(): loopen, n=%d\n", n);fflush(stdout); DEBUG
{ int i = n;
while (i--) {
  printf("user=%s; id=%s, line=%s; host=%s; pid=%d; type=%d; time=%s\n", 
    this[i].ut_user, this[i].ut_id, this[i].ut_line, this[i].ut_host, this[i].ut_pid, this[i].ut_type, ctime(&this[i].ut_time));
}
i = n * sizeof(struct utmp);
printf("i=%d\n", i);
write(1, &this, i);
}*/
  ...
  while (n--)
#ifndef _MINIX /* Body of while for Minix follows to avoid a greater mess. */
    {
      if (UT_USER (this) [0]
#ifdef USER_PROCESS
...
#else /* _MINIX; this is the code for Minix. It searches backwards. */
      if (this[n].ut_type == USER_PROCESS) { /* login */
          ++entries;
      } else if (this[n].ut_type == DEAD_PROCESS) { /* logout; ut_name empty */
          --entries;
      } else if (this[n].ut_line[0] == '~' && this[n].ut_line[1] == '\0' && 
         !strcmp(this[n].ut_name, "reboot")) { /* boot; ut_type == BOOT_TIME */
          boot_time = this[n].ut_time;
          break;
      }

    if (entries < 0) {
        error (EXIT_FAILURE, errno, _("#logouts > #logins since last reboot"));
    }
#endif /* _MINIX */
