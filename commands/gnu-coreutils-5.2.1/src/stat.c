...
#  include <nfs/vfs.h>
# endif
#elif HAVE_SYS_STATFS_H /* Minix; contains only f_bsize field. */
...
# ifdef _MINIX
  switch (0) /* Minix has no f_type field; type set below for Minix. */
# elif
...
# elif __GNU__
    ...
    case FSTYPE_ISO9660:
      type = "iso9660";
      break;
# elif _MINIX /* Minix only supports its own fs. */
    case 0:
      type = "minix v3";
      break;
# endif
...
# ifndef _MINIX /* Minix doesn't like f_type. */
  {
...
print_stat (char *pformat, char m, char const *filename, void const *data)
{
  ...
  switch (m)
    ...
    case 'o':
      strcat (pformat, "d");
#ifdef _MINIX /* no st_blksize field, use a constant guess. */
      printf (pformat, 4096);
#elif
...
#ifdef _MINIX /* Minix only has fstatfs. */
  int fd, i;

  fd = open(filename, O_RDONLY);
  if (fd < 0) i == -1;
  else {
      i = fstatfs (fd, &statfsbuf);
      close(fd);
  }
#elif
