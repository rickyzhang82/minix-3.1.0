/* $Id: atlin.c,v 1.1 2005/09/01 12:13:28 beng Exp $ */

#define DEBUGGEE
#include "message.h"
#include <signal.h>		/* signal() */
#if 0
#include <unistd.h>		/* sbrk(),read(),write(),brk() */
#endif
...
/* c.f. lib.e */
char *__Get_LB(int);
char *__Get_Hol0(void);
char *__Get_PC(char *);
char *__Cvt_LB_AB(char *);
char *__Get_HP(void);
void __Set_HP(char *);

/* c.f. par_misc.e */
int ___topsave(void);
void ___topload(char *);

/* Machine-dependent support */
void __Set_PC(char *,char *);

char *_sbrk(int);
extern int  etext;
static void send_ok(int);

/* Memory layouts differ depending on whether the text and
 * data segments are common, or separate.
 */
#ifdef COMMON_TEXTDATA
#define	check_ptr(c)	(((c) >= (char *)&etext && (c) <= currbrk) || (c) >= LB)
#else
#define	check_ptr(c)	((c) <= currbrk || (c) >= LB)
#endif
...
static int
sendreply(struct message_hdr *m, char *p, long sz)
{
...
void
___sendtop(char *addr, char *SP, unsigned int sz)
{
