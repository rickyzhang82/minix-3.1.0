 ...
 */
/* $Id: out.h,v 1.3 2005/09/01 12:13:17 beng Exp $ */
 ...
 *   0000:  struct outhead
 *          struct outsect     (#oh_nsect)
 *          struct outrelo     (#oh_nrelo)
 *          struct outname     (#oh_nname)
 *          <string area>
 */
...
/* ACK supports simple relocations; or_addr is the address of the
 * relocation datum expressed as an offset from the base of the section
 * identifed by or_sect (which must be in the range S_MIN to S_MAX - it
 * doesn't make sense to have relocations in S_UND or S_ABS). The field
 * or_nami is typically the index of the referenced field in the symbol
 * table, in which case the reference is to an undefined external symbol,
 * a common symbol, or a section name, and the relocatable datum (as
 * located by or_sect/or_addr) contains an offset from the indicated
 * symbol or section.
 *
 * If or_nami==oh_nname, then the relocatable datum is an absolute number.
 *
 * If RELPC is expressed, adjust by the difference of the referencing
 * and referenced sections.
 *
 * See util/led/relocate.c for the precise calculations, and as/comm6.c
 * for relocation support in the assembler framework.
 */
struct outrelo {
...
/*
 * section type bits and fields (on_type)
 */
...
/*
 * S_TYP field values (also used in or_sect)
 */
