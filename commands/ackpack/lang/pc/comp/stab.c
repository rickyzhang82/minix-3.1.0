...
/* $Id: stab.c,v 1.1 2005/09/01 12:13:24 beng Exp $ */
...
static
void
stb_type(struct type * tp,int assign_num)
{
	char buf[128];
	...
	case T_SET:
		addc_db_str('S');
		stb_type(tp->next, 0);
		print_db_str(buf, ";%ld;%ld;", (long) tp->tp_size, 0L);
		break;
	case T_ARRAY:
		addc_db_str('a');
		if (IsConformantArray(tp)) {
			addc_db_str('r');
			stb_type(tp->next, 0);
			print_db_str(buf, ";A%ld;Z%ld", (long) tp->arr_cfdescr, (long) tp->arr_cfdescr);
		}
		else {
			stb_type(tp->next, 0);
		}
	...
	case T_ENUMERATION:
		...
		break;
	case T_RECORD:
		print_db_str(buf, "s%ld", (long) tp->tp_size);
		{
...
void
stb_addtp(char * s,struct type * tp)
{
	create_db_str();
	adds_db_str(s);
	addc_db_str(':');
...
void
stb_string(struct def * df,long kind)
{
	...
	addc_db_str(':');
	if (kind == D_MODULE) {
		print_db_str(buf, "M%d;", df->prc_vis->sc_count);
	...
	case D_PROCEDURE:
	case D_FUNCTION:
		print_db_str(buf, "Q%d;", df->prc_vis->sc_count);
		stb_type(tp->next ? tp->next : void_type, 0);
		addc_db_str(';');
	...
	case D_CONST:
		...
		case T_POINTER:
		case T_PROCEDURE:
			print_db_str(buf, "i%ld;", (long) df->con_const->nd_INT);
			break;
		case T_CHAR:
			print_db_str(buf, "c%ld;", (long) df->con_const->nd_INT);
			break;
		case T_REAL:
			addc_db_str('r');
		...
		case T_ENUMERATION:
			addc_db_str('e');
			stb_type(tp, 0);
			print_db_str(buf, ",%ld;", (long) df->con_const->nd_INT);
			break;
		case T_SET: {
			register int i;
