...
STATIC
void
Xerror(struct node * nd,char * mess)
{
...
int
ChkLinkOrName(struct node * expp)
{
...
STATIC int
ChkExLinkOrName(struct node * expp)
{
...
STATIC int
ChkElement(struct node * expp, struct type ** tp, arith ** set, unsigned int * cnt)
{
...
char *
ChkAllowedVar(struct node * nd, int reading)		/* reading indicates read or readln */
{
	char *message = 0;
...
STATIC
int
ChkVarPar(struct node * nd, struct node * name)
{
	/* 	ISO 6.6.3.3 :
...
STATIC struct node *
getarg(struct node ** argp, int bases, int varaccess, struct node * name, struct type * paramtp)
{
	...
	*/
	for( param = ParamList(left->nd_type); param; param = param->next ) {
		...
		if( !TstParCompat(TypeOfParam(param), left->nd_type,
			...
			sprintf(ebuf, "type incompatibility in parameter %d",
					cnt);
			Xerror(name, ebuf);
			retval = 0;
		}
...
STATIC int
ChkNameOrCall(struct node * expp)
{
...
STATIC int
ChkStandard(struct node * expp, struct node * left)
{
...
STATIC int
done_before(struct node * ignored)
{
	return 1;
}

STATIC int
no_var_access(struct node * expp)
{
