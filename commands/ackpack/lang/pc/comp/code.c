...
static
void
RegisterMessages(struct def * df)
{
...
void
CodeEndBlock(struct def * df,arith StackAdjustment)
{
...
static
void
Operands(struct node * leftop,struct node * rightop)
{
	CodePExpr(leftop);
	CodePExpr(rightop);
}
...
static
void
CodeBoper(struct node * expr,label true_label)
	/* expr -- the expression tree itself	*/
	/* true_label -- label to jump to in logical exprs */
{
...
static
void
CodeEl(struct node * nd,struct type * tp)
{
...
void
Int2Real(arith size)		/* size is different for integers and longs */
{
...
static
void
CodeConfDescr(struct type * ftp,struct type * atp)
{
...
static
void
CodePString(struct node * nd,struct type * tp)
{
	/* no null padding */
...
void
RangeCheck(struct type * tpl,struct type * tpr)
{
...
static
struct type *
CodeParameters(struct paramlist * param,struct node * arg)
{
...
void
CodeExpr(struct node * nd,struct desig * ds,label true_label)
{
	register struct type *tp = nd->nd_type;
