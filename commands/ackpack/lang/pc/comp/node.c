...
struct node *
MkNode(int class,struct node * left,struct node * right,struct token * token)
{
...
struct node *
MkLeaf(int class,struct token * token)
{
	register struct node *nd = new_node();
...
int
NodeCrash(struct node * expp)
{
	crash("Illegal node %d", expp->nd_class);
	/* Never returns, but conform to interface */
	return 0;
}
...
static
void
printnode(struct node * nd,int lvl)
{
	indnt(lvl);
