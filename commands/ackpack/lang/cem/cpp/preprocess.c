 ...
 */
/* $Id: preprocess.c,v 1.1 2005/09/01 12:13:20 beng Exp $ */
/* PREPROCESSOR DRIVER */
...
#define	do_line_dir(lineno, fn)						\
	...
	for (;;) {
		...
		while (startline) {
			...
			register int i = 0;
			int LiNo = LineNumber;
			const char *FiNam = FileName;

			while (i < pragma_nr) {
			    register const char *c_ptr = "#pragma";
...
static
char *
SkipComment(char *op,int *lineno)
{
