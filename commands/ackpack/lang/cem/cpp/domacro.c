 ...
 */
/* $Id: domacro.c,v 1.1 2005/09/01 12:13:20 beng Exp $ */
...
extern char options[];
extern	const char **inctable;	/* list of include directories		*/
...
do_include(void)
{
	...
	*/
	...
	if (filenm) {
		if (!InsertFile(filenm, &inctable[tok==FILESPECIFIER?1:0],&result)){
...
static
int
getparams(char *buf[],char parbuf[])
{
...
void
macro_def(struct idf *id,char *text,int nformals,int length,int flags)
{
...
#ifdef DOBITS
		register const char *p = id->id_text;
...
static
char *
get_text(char *formals[],int *length)
{
