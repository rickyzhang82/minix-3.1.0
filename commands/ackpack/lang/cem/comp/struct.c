 ...
 */
/* $Id: struct.c,v 1.1 2005/09/01 12:13:20 beng Exp $ */
...
static arith add_field(arith *szp,struct field *fd,struct type **fdtpp,struct idf *idf,struct type *stp);
...
/* this is horrible */
void
add_sel(struct type *stp,struct type *tp,struct idf *idf,
		struct sdef ***sdefpp,arith *szp,struct field *fd)
#if 0
...
static
void
check_selector(struct idf *idf,struct type *stp)
...
void
declare_struct(int fund,struct idf *idf,struct type **tpp)
{
...
void
apply_struct(int fund,struct idf *idf,struct type **tpp)
{
...
struct sdef *
idf2sdef(struct idf *idf,struct type *tp)
{
...
#if	0
int
uniq_selector(struct sdef *idf_sdef)
{
...
/* This must be included even when NOBITFIELD is defined */
/* See code above - handling UNION */
#if 1 /* ndef NOBITFIELD [MK 20031012] */
static
arith
add_field(arith *szp,struct field *fd,struct type **fdtpp,struct idf *idf,struct type *stp)
#if 0
