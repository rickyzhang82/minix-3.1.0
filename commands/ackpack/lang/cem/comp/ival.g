 ...
 */
/* $Id: ival.g,v 1.1 2005/09/01 12:13:19 beng Exp $ */
...
static void gen_tpcheck(struct type **tpp);
static void gen_simple_exp(struct type **,struct expr **);
static struct type ** gen_tphead(struct type **tpp,int nest);
static struct type ** gen_tpmiddle(void);
static struct sdef * gen_align_to_next(struct e_stack *p);
static void gen_tpend(void);
static void check_and_pad(struct expr **expp,struct type **tpp);
static void pad(struct type *tpx);
static void check_ival(struct expr **expp,struct type *tp);
static void ch_array(struct type **tpp,struct expr *ex);
#ifndef NOBITFIELD
static void put_bf(struct type *tp,arith val);
#endif
static int zero_bytes(struct sdef *sd);
static int valid_type(struct type *tp,char *str);
static void con_int(struct expr *ex);
static void illegal_init_cst(struct expr *ex);
static void too_many_initialisers(void);
...
static
void
gen_simple_exp(struct type **tpp,struct expr **expp)
{
	register struct type *tp;
...
static
struct type **
arr_elem(struct type **tpp,struct e_stack *p)
{
...
static
struct sdef *
next_field(struct sdef *sd,struct e_stack *p)
{
	if (sd->sd_sdef)
...
static
struct type **
gen_tphead(struct type **tpp,int nest)
{
	register struct type *tp = *tpp;
	...
	if (tp->tp_fund == UNION) {
		 ...
		 * initialiser. It could, however, also be the start of the
		 * initialiser for the first union field...
		 */
		sd = tp->tp_sdef;
		if (AHEAD != '{' &&
		    (aggregate_type(sd->sd_type) || 
		     sd->sd_type->tp_fund == UNION)) {
				 ...
				 * initialiser of the union field, so:
				 */
...
static
struct sdef *
gen_align_to_next(struct e_stack *p)
{
...
static
void
check_and_pad(struct expr **expp,struct type **tpp)
{
	register struct type *tp = *tpp;
...
static
void
check_ival(struct expr **expp,struct type *tp)
{
...
static
void
ch_array(struct type **tpp,struct expr *ex)
...
*/
void
str_cst(char *str,int len,int inrom)
{
...
static
void
put_bf(struct type *tp,arith val)
{
...
static
int
valid_type(struct type *tp,char *str)
{
...
static
void
illegal_init_cst(struct expr *ex)
{
