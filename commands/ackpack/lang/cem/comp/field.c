 ...
 */
/* $Id: field.c,v 1.1 2005/09/01 12:13:19 beng Exp $ */
...
static void store_field(struct field *,int,int,struct expr *,arith);
...
*/
void
eval_field(struct expr *expr,int code)
{
	int op = expr->OP_OPER;
...
static
void
store_field(struct field *fd,int uns,int code,struct expr *leftop,arith tmpvar)
{
	C_loc(fd->fd_mask);
	C_and(word_size);
	if (code == TRUE)
