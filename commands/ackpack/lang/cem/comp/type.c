 ...
 */
/* $Id: type.c,v 1.1 2005/09/01 12:13:20 beng Exp $ */
...
#ifndef NOBITFIELD
static struct type * field_of(struct type *tp,int qual);
#endif
...
struct type *
construct_type(int fund,struct type *tp,int qual,arith count,struct proto *pl)
#if 0
...
struct type *
function_of(struct type *tp,struct proto *pl,int qual)
{
#if 0
/* See comment below */
...
struct type *
pointer_to(struct type *tp,int qual)
{
...
struct type *
array_of(struct type *tp,arith count,int qual)
{
...
static
struct type *
field_of(struct type *tp,int qual)
{
...
arith
size_of_type(struct type *tp,char nm[])
{
	arith sz = tp->tp_size;
...
void
idf2type(struct idf *idf,struct type **tpp)
{
...
struct type *
standard_type(int fund,int sgn,int algn,arith sz)
{
