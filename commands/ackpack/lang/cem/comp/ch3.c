 ...
 */
/* $Id: ch3.c,v 1.1 2005/09/01 12:13:19 beng Exp $ */
...
static int equal_proto(struct proto *,struct proto *,int);
static int legal_mixture(struct type *,struct type *,int);
...
void
ch3sel(struct expr **expp,int oper,struct idf *idf)
{
...
void
ch3incr(struct expr **expp,int oper)
{
...
void
ch3cast(struct expr **expp,int oper,struct type *tp)
{
...
*/
int
equal_type(struct type *tp,struct type *otp,int qual_lev,int diag)
{
	 if (tp == otp)
		return 1;
	if (!tp
...
int
check_pseudoproto(struct proto *pl,struct proto *opl,int diag)
{
	int retval = 1;
...
static
int
legal_mixture(struct type *tp,struct type *otp,int diag)
{
...
static
int
equal_proto(struct proto *pl,struct proto *opl,int diag)
{
	if (pl == opl)
		return 1;
...
int
recurqual(struct type *tp,int qual)
{
...
void
ch3asgn(struct expr **expp,int oper,struct expr *expr)
{
...
*/
int
is_integral_type(struct type *tp)
{
	switch (tp->tp_fund)	{
	case CHAR:
	case SHORT:
