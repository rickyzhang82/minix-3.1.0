 ...
 */
/* $Id: dumpidf.c,v 1.1 2005/09/01 12:13:19 beng Exp $ */
/*	DUMP ROUTINES	*/
...
void
dumpidf(struct idf *idf,int opt)
{
...
void
dumpdefs(struct def *def,int opt)
{
	dumplevel++;
...
void
dumpsdefs(struct sdef *sdef,enum sdef_kind sdk)
{
...
void
print_expr(char msg[],struct expr *expr)
{
...
void
p1_expr(int lvl,struct expr *expr)
{
	p1_indent(lvl);
	if (!expr)	{
		print("NILEXPR\n");
