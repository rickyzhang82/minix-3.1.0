 ...
 */
/* $Id: l_misc.c,v 1.1 2005/09/01 12:13:19 beng Exp $ */
...
void
check_hiding(struct idf *idf,int lvl,int sc)
{
...
void
lint_new_oper(struct expr *expr)
{
...
PRIVATE
void
lint_enum_arith(int l_fund,int oper,int r_fund)
{
	if (	l_fund == ENUM
	&&	r_fund != CHAR
	&&	r_fund != SHORT
...
PRIVATE
void
lint_conversion(struct expr *from_expr,int to_fund)
{
...
void
lint_ret_conv(struct expr *from_expr)
{
	lint_conversion(from_expr, func_type->tp_fund);
}

void
lint_ptr_conv(short from,short to)
{
...
void
lint_relop(struct expr *left,struct expr *right,int oper)
