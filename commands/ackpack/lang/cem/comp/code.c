 ...
 */
/* $Id: code.c,v 1.1 2005/09/01 12:13:19 beng Exp $ */
...
static void prc_entry(const char *);
static void prc_exit(void);


#ifndef	LINT
void
init_code(char *dst_file)
{
...
void
def_strings(struct string_cst *sc)
{
	while (sc) {
...
void
code_scope(const char *text,struct def *def)
{
...
static const char *last_fn_given;
static label file_name_label;

void
begin_proc(struct decspecs *ds,struct idf *idf)		/* to be called when entering a procedure */
{
	...
	*/
	register const char *name = idf->id_text;
...
void
do_return_expr(struct expr *expr)
{
...
void
code_declaration(struct idf *idf,struct expr *expr,int lvl,int sc)
#if 0
...
void
loc_init(struct expr *expr,struct idf *id)
{
...
void
formal_cvt(int hasproto,struct def *df)
{
...
#endif	/* LINT */
void
code_expr(struct expr *expr,int val,int code,label tlbl,label flbl)
{
...
void
stack_stmt(label break_label,label cont_label)
{
