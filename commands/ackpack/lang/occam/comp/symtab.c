/* $Id: symtab.c,v 1.1 2005/09/01 12:13:23 beng Exp $ */
/*
...
static
struct symbol **
search_sym(struct symbol **tree,char *name)
...
struct symbol *
insert(char *name,int type,int arr_siz,union type_info *info)
...
void
check_recursion(struct expr *proc)
{
...
void
var_memory(union type_info *info,int type,int n)
...
void
chan_memory(union type_info *info,int n)
{
	info->vc.st.level=curr_level;
...
void
pars_add(struct par_list ***aapars,int type,struct symbol *var)
...
int
form_offsets(struct par_list *pars)
