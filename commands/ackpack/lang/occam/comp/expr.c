/* $Id: expr.c,v 1.1 2005/09/01 12:13:22 beng Exp $ */
/*
...
static
void
assignable(struct expr *l,struct expr *r)
...
static
void
subscriptable(struct expr *l,struct expr *r,int byte,int *atype,int *arr_siz)
...
struct expr *
new_node(int op,struct expr *left,struct expr *right,int byte)
...
struct expr *
new_table(int kind,struct table *tab)
...
struct expr *
new_io(int out,struct expr *chan,struct expr_list *args)
...
struct expr *
new_call(struct expr *proc,struct expr_list *args)
...
void
table_add(struct table ***aapt,long val)
...
void
expr_list_add(struct expr_list ***aaelp,struct expr *arg)
...
void
check_io(int out,struct expr *arg)
{
	if (out)
		outputable(arg);
	else
...
void
check_param(struct par_list **aform,struct expr *act,int *err)
