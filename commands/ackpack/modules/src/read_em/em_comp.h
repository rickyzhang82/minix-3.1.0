 ...
 */
/* $Id: em_comp.h,v 1.3 2005/09/01 12:13:27 beng Exp $ */
...
struct e_arg {
	...
	union e_simple_arg {
		...
		const char	*emu_dnam;	/* a data label */
		const char	*emu_pnam;	/* a procedure name */
		const char	*emu_string;	/* a string (fcon,icon,ucon,scon) */
	}	ema_arg;
	arith	ema_szoroff;
};
...
int EM_open(const char *);
void EM_close(void);
int EM_getinstr(struct e_instr *);
int EM_mkcalls(struct e_instr *);
