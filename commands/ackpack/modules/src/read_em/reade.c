/* $Id: reade.c,v 1.3 2005/09/01 12:13:27 beng Exp $ */
/*
...
PRIVATE void gettyp(int,struct e_arg *);

PRIVATE
int
offsetted(int argtyp, arith * ap)
{
	register int c;
...
PRIVATE
int
getnumber(int c, struct e_arg * ap)
{
	char str[256 + 1];
	register char *p = str;
	int n;
	...
	if (c == '+' || c == '-') {
		...
		/* Allow spaces between sign and first digit */
		while (' ' == c )
			c = getbyte();
	}
...
PRIVATE int getexpr(int,struct e_arg *);

PRIVATE
int
getfactor(int c, struct e_arg * ap)
{
	if (c == '(') {
...
PRIVATE
void
gettyp(int typset, struct e_arg * ap)
{
	register int c, t;
	register int argtyp;
...
PRIVATE
void
getarg(int typset, struct e_arg * ap)
{
	register int c;
...
PRIVATE
void
getmnem(int c, struct e_instr * p)
{
	register int h;
	int i;
	register struct string *s;
...
PRIVATE
void
getlabel(int c, struct e_instr * p)
{
