/* $Id: read_em.c,v 1.3 2005/09/01 12:13:27 beng Exp $ */
/*
...
#define ungetbyte(uch)	(void)((uch) >= 0 && (0!=(*--_ich = (uch))))
...
_fill(void)
{
	static char text[ACK_BUFSIZ + 1];
	static size_t sz;

	if (_ich && _ich < &text[sz]) return _ich++, '\0';
	_ich = text;
	if (sys_read(fd, text, ACK_BUFSIZ, &sz) &&
	    sz > 0
	   ) {
		text[sz] = '\0';
		return (*_ich++&0377);
...
#ifdef CHECKING
static const char *argrange = "Argument range error";
#define check(expr) do { if(!(expr) && !EM_error) EM_error = argrange; } while( 0 )
#else /* not CHECKING */
