/* $Id: insert.c,v 1.3 2005/09/01 12:13:25 beng Exp $ */
...
getbyte(long b)
{
	...
	*/
	...
	if (b < start_core || b >= curr_pos) {
		...
		if (! C_ibuf) {
			C_ibuf = Malloc(ACK_BUFSIZ);
		}
		if (sys_read(C_tfr, C_ibuf, ACK_BUFSIZ, &n) == 0) {
			C_failed();
		}
		curr_pos += n;
		start_core = nb;
	...
	if (C_tmpfile == 0) {
		...
		if( 0 == C_tmpdir[ 0 ] )
		{
			...
			if( 0 == tmp )
				tmp = getenv( "TEMP" );
			if( 0 == tmp )
				tmp = "/tmp";	/* Last resort - safe on unix */
			strcpy( C_tmpdir, tmp );
		}
