/* $Id: wr.c,v 1.3 2005/09/01 12:13:26 beng Exp $ */
/*
...
#endif
{
	...
	ptr->currpos = o;
#else
	sys_seek( ptr->fd, o, 0, & ptr->currpos );
#endif /* OUTSEEK */
...
#ifdef OUTSEEK
	if ((outfile = open(f, O_WRONLY)) < 0)
		return 0;
#ifdef _WIN32
	setmode( outfile, _O_BINARY );
#endif
	currpos = 0;
#else /* not OUTSEEK */
...
wr_ohead(struct outhead *head)
{
	...
	if (ACK_BYTE_ORDER != 0x0123 || sizeof(struct outhead) != SZ_HEAD)
	{
		char buf[SZ_HEAD];
...
void
wr_sect(struct outsect *sect,unsigned int cnt)
{
...
void
wr_outsect(int s)	/* section number */
{
	...
	if (s != sectionnr && s >= (SECTCNT-1) && sectionnr >= (SECTCNT-1)) {
		...
		if (offset[s] != ptr->currpos) {
			sys_seek( ptr->fd, offset[s], 0, & ptr->currpos );
#ifdef OUTSEEK
			currpos = ptr->currpos;
#endif
		}
 ...
 */
void
wr_emit(char *emit,long cnt)
{
	OUTWRITE(PARTEMIT + getsect(sectionnr) , emit, cnt);
}

void
wr_relo(struct outrelo *relo,unsigned int cnt)
{
#if ACK_BYTE_ORDER == 0x0123
...
void
wr_name(struct outname *name,unsigned int cnt)
{
#if ACK_BYTE_ORDER == 0x0123
