 ...
 */
#ifndef lint
static char rcsid[] = "$Id: extract.c,v 1.3 2005/09/01 12:13:33 beng Exp $";
#endif
...
static
void
transfer(struct outname *src,struct outname *dst)
{
...
static
void
redefine(struct outname *new,struct outname *old)
{
	if (!ISCOMMON(old)) {
		if (!ISCOMMON(new))
...
static
void
getexternal(struct outname *name)
{
 ...
 */
void
namerelocate(struct outname *name)
{
