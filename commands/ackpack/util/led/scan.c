 ...
 */
#ifndef lint
static char rcsid[] = "$Id: scan.c,v 1.3 2005/09/01 12:13:34 beng Exp $";
#endif
...
void
get_archive_header(struct ar_hdr *archive_header)
{
	if (passnumber == FIRST || !incore) {
		rd_arhdr(infile, archive_header);
	} else {
...
static
bool
direct_alloc(struct outhead *head)
{
...
static
bool
indirect_alloc(struct outhead *head)
{
 ...
 */
void
skip_modul(struct outhead *head)
{
 ...
 */
char *
getemit(struct outhead *head,struct outsect *sects,int sectindex)
{
	char		*ret;
	ind_t		off;
...
char *
getblk(long totalsz,long *pblksz,int sectindex)
{
	char	*ret;
	long	sz = (1L << 30);
