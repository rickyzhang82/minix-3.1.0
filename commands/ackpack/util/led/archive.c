 ...
 */
#ifndef lint
static char rcsid[] = "$Id: archive.c,v 1.3 2005/09/01 12:13:33 beng Exp $";
#endif
...
getsymdeftable(void)
{
	...
	rd_ranlib(infile, (struct ranlib *)address(ALLORANL, off), count);

	nchar = rd_long(infile);
	debug("%ld ranlib chars\n", nchar, 0, 0, 0);
	off = hard_alloc(ALLORANL, nchar);
	if (off == BADOFF)
		fatal("no space for ranlib strings");
	rd_bytes(infile, address(ALLORANL, off), nchar);

	/* Only keep one ranlib table in core */
	ran = (struct ranlib *)address(ALLORANL, 0);
	while (count--) {
		/*
