...
int
findstructel(int number,string name,int *t)
{
	register i;
	register token_p tp;
...
int
lookup(int comm, int operator, int lnode, int rnode)
{
	register node_p p;
...
ident_p
ilookup(string name, int enterf)
{
...
int
formconversion(char * p, token_p tp)
{
	char buf[256];
	register char *q=buf;
	char field[256];
...
void
setfields(token_p tp,string format)
{
	register i;
	list2 ll;
	register list1 l;
...
int
commontype(expr_t e1, expr_t e2)
{
