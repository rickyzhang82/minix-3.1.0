 ...
 */
#ifndef NORCSID
static char rcsid[]= "$Id: expr.c,v 1.3 2005/09/01 12:13:34 beng Exp $";
#endif
...
expr_t
make_expr(int type,int operator,int op1,int op2)
{
	expr_t result;
...
expr_t
subreg_expr(int tokarg,int subreg)
{
	expr_t result;
...
void
subregset(unsigned short *sp,int subreg,unsigned short *regset)
{
	register i;
	register reginfo *rp;
...
int
membset(int setno,char *name,unsigned short *regset,char *appearance,int restyp,int *typp)
{
...
expr_t
memb_expr(int setno,char *name,char *appearance,int tokarg)
{
	expr_t result;
	int res_j;
...
expr_t
tokm_expr(int tokarg,char *name)
{
	char app[100];
...
expr_t
all_expr(int all_no,int subreg)
{
	set_t localset;
	register i;
	register unsigned short *sp;
...
expr_t
regvar_expr(expr_t e,int regtyp)
{
	expr_t result;
	register i;
...
int
ex_lookup(int operator,int lnode,int rnode)
{
	register node_p p;
