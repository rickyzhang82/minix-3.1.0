 ...
 */
#ifndef NORCSID
static char rcsid[]= "$Id: hall.c,v 1.3 2005/09/01 12:13:34 beng Exp $";
#endif
...
static int recurhall(int,unsigned short[][SETSIZE]);
static void unite(unsigned short*,unsigned short*);

#define MAXHALL (TOKPATMAX+MAXALLREG)
unsigned short hallsets[MAXHALL][SETSIZE];
int nhallsets= -1;
...
int
card(unsigned short *sp)
{
	register sum,i;
	
	sum=0;
	for(i=0;i<8*sizeof(unsigned short)*SETSIZE;i++)
		if (BIT(sp,i))
			sum++;
	return(sum);
}
...
static
int
recurhall(int nhallsets,unsigned short hallsets[][SETSIZE])
{
	unsigned short copysets[MAXHALL][SETSIZE];
	unsigned short setsum[SETSIZE];
...
static
void
unite(unsigned short *sp,unsigned short *into)
{
	register i;
