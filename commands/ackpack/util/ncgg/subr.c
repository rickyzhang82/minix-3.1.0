 ...
 */
#ifndef NORCSID
static char rcsid[]= "$Id: subr.c,v 1.3 2005/09/01 12:13:34 beng Exp $";
#endif
...
int
n_reg(char *name,char *printstring,int nmemb,int member1,int member2)
{
	register symbol *sy_p;
...
void
regline(varinfo *rl,varinfo *pl,int rv)
{
...
void
freevi(struct varinfo *vip)
{
/*	register i; */

#if 0
	/* I suspect this is being used to find the end of the
	 * text segment, i.e. to determine if a block has been
	 * allocated dynamically or not. Accept a memory leak
	 * on modern systems.
	 */
	extern char *end;
#endif
...
int
chkincl(int value,int lwb,int upb)
{
...
int
subset(unsigned short *sp1,unsigned short *sp2,int setsize)
{
	register i;
