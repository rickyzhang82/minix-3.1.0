 ...
 */
#ifndef NORCSID
static char rcsid[]= "$Id: iocc.c,v 1.3 2005/09/01 12:13:34 beng Exp $";
#endif
...
expr_t iextoaddr(expr_t);
static int instalookup(inst_t,int);


iocc_t
subr_iocc(int tokarg,int subreg)
{
	inst_t insta;
	iocc_t  result;
	register i;
...
iocc_t
tokm_iocc(int tokarg,char *ident)
{
	iocc_t result;
	inst_t insta;
	register i;
...
iocc_t
all_iocc(int all_no,int subreg)
{
	iocc_t result;
	inst_t insta;
	register i;
...
static
int
instalookup(inst_t insta,int filled)
{
	register i,j;
