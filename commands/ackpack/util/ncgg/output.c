...
#ifndef NORCSID
static char rcsid[]= "$Id: output.c,v 1.3 2005/09/01 12:13:34 beng Exp $";
#endif
...
#ifdef _WIN32
# include <io.h>	/* creat(), write(), close() */
#endif
...
initio(void)
{
	...
	fprintf(ctable,"#ifdef _MSC_VER\n#pragma warning(once:4305)\n#endif\n");

	if (code_in_c)
		fprintf(ctable,"unsigned char coderules[] = {");
	else
...
outars(void)
{
	...
	fprintf(ctable,"unsigned char pattern[%d]={\n",npatbytes+1);
...
void
dopattern(int stackcoerc,varinfo *kills,varinfo *allocates,
...
void
used(char * resource,int use,int max)
{
