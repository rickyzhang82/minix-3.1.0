/* $Id: print.c,v 1.1 2005/09/01 12:13:33 beng Exp $ */
...
static
void
print_unsigned(p_type tp, long v, char *format)
{
  while (format && *format) {
...
static
void
print_literal(p_type tp, long v, int compressed, char *format)
{
...
static
void
print_integer(p_type tp, long v, char *format)
{
  while (format && *format) {
	if (strindex("cdohx", *format)) break;
	format++;
...
void
print_params(p_type tp, t_addr AB, int static_link)
{
  char *param_bytes;
  register char *p;
  register int i;
...
void
print_val(p_type tp, long tp_sz, char *addr,
         int compressed, int indent, char *format)
#if 0
