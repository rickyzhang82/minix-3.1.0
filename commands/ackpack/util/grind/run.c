/* $Id: run.c,v 1.1 2005/09/01 12:13:33 beng Exp $ */
...
static int
ugetm(struct message_hdr *message)
{
...
static int
uputm(struct message_hdr *message)
{
...
static void
catch_sigpipe(int ignored)
{
  child_pid = 0;
}

static int
stopped(char *s, t_addr a, int stop_reason)
#if 0
  char	*s;		/* stop message */
...
static int
could_send(struct message_hdr *m, int stop_message)
{
  int	type;
  t_addr a;
  static int level = 0;
...
static int
getbytes(long size, t_addr from, char *to, int kind, int errmess)
{
  struct message_hdr	m;
...
int
get_bytes(long size, t_addr from, char *to)
{
  return getbytes(size, from, to, M_GETBYTES, 1);
}

int
get_string(long size, t_addr from, char *to)
{
...
void
set_bytes(long size, char *from, t_addr to)
{
  struct message_hdr	m;
...
t_addr
get_dump(char **globbuf, char **stackbuf)
{
  struct message_hdr	m;
...
int
put_dump(char *globbuf, char *stackbuf)
{
  struct message_hdr m;
...
int
set_or_clear_breakpoint(t_addr a, int type)
{
  struct message_hdr m;
...
int
set_or_clear_trace(t_addr start, t_addr end, int type)
{
  struct message_hdr m;
