/* $Id: symbol.hh,v 1.1 2005/09/01 12:13:33 beng Exp $ */
...
p_symbol NewSymbol(char *, struct scope *, int, struct outname *);
p_symbol Lookup(struct idf *, struct scope *, int);
p_symbol Lookfromscope(struct idf *, int, struct scope *);
p_symbol add_file(char *);
p_symbol identify(p_tree, int);
int consistent(p_tree, struct scope *);
void pr_sym(p_symbol);
void resolve_cross(struct type *);
