/* $Id: rd.c,v 1.1 2005/09/01 12:13:33 beng Exp $ */

/* a.out file reading for supported systems:
 *     !solaris && (sun || vax)
 *     i386 && xenix
 *     solaris
 *
 * For each system, have the functions:
 *     int   rd_open(char*)
 *     void  rd_ohead(struct outhead *h)
 *     void  rd_name(struct outname *names, unsigned int count)
 *     void  rd_string(char *strings, long count)
 *     void  rd_close(void)
 *
 * These signatures match those defined in modules/src/object/object.h;
 * unsupported systems will link against the standard ACK implementation
 * of these functions.
 *
 * Note that a different mechanisn is used for Minix - the Minix executable
 * format truncates the debug symbol strings, and hence, a separate ACK
 * format debug symbol file is created.
 */
...
/*ARGSUSED1*/
void
rd_name(struct outname *names, unsigned int count)
#if 0
...
void
rd_string(char *strings, long count)
{
#if defined(sun)
  if (bh.a_magic == ZMAGIC) bh.a_text += sizeof(struct exec);
#endif
...
static 
void
get_names(struct outhead *h, long sz)
{
  register char	*xnms = malloc((unsigned) sz);
  register char *p;
  register struct outname *onm = (struct outname *)
                           malloc((((unsigned)sz+8)/9)*sizeof(struct outname));
  struct  xnm {
...
void
rd_name(struct outname *nm, unsigned int count)
{
