/* $Id: il_aux.c,v 1.3 2005/09/01 12:13:31 beng Exp $ */
/*
...
void
remunit(short kind,proc_p p,line_p l)
{
	register bblock_p b;
	bblock_p next;
	Lindex   pi;
...
getcall(FILE *cf)
{
	...
	while(n--) {
		...
		act->ac_size = getoff();
		act->ac_inl = getbyte();
		act->ac_exp = getlines(cf,m,&voided,0);
		*app = act;
		app = &act->ac_next;
	}
	*app = (actual_p) 0;
...
STATIC
void
putactuals(actual_p alist,FILE *cfile)
{
...
void
putcall(call_p c,FILE *cfile,short level)
{
	/* output a call */
