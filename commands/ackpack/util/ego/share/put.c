/* $Id: put.c,v 1.3 2005/09/01 12:13:32 beng Exp $ */
/*
...
void
putdtable(dblock_p head,FILE *df)
{
...
void
putptable(proc_p head,FILE *pf,bool all)
{
	register proc_p p;
	proc_p next;
	register short n = 0;
...
STATIC
void
outid(Lelem_t e,void (*p)(Lelem_t))
{
...
void
putunit(short kind,proc_p p,line_p l,FILE *gf,FILE *lf)
{
	register bblock_p b;
	register short n = 0;
	Lindex   pi;
	...
	for (b = p->p_start; b != (bblock_p) 0; b = b->b_next) {
		...
		outlset(b->b_succ, (void (*)(Lelem_t))outblock); /* put succ set */
		outlset(b->b_pred, (void (*)(Lelem_t))outblock); /* put pred set */
		outblock(b->b_idom); /* put id of immediate dominator */
		outlset(b->b_loops, (void (*)(Lelem_t))outloop); /* put loop set */
		outshort(b->b_flags);
	}
