/* $Id: get.c,v 1.3 2005/09/01 12:13:32 beng Exp $ */
/*
...
STATIC
void
argstring(short length,argb_p abp)
{
...
line_p
getlines(FILE *lf,int n,proc_p *p_out,bool collect_mes)
{
...
bool
getunit(FILE *gf,FILE *lf,short *kind_out,bblock_p *g_out,line_p *l_out,
		proc_p *p_out,bool collect_mes)
{
	...
	*kind_out = LTEXT;
	...
	for (i = 0; i < blength; i++) {
		...
		b->b_succ = getlset((void*(*)(int))getblock);
		b->b_pred = getlset((void*(*)(int))getblock);
		b->b_idom = getblock(getshort());
		b->b_loops = getlset((void*(*)(int))getloop);
		b->b_flags = getshort();
