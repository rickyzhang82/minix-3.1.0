/* $Id: ra.c,v 1.3 2005/09/01 12:13:31 beng Exp $ */
/*
...
void
get_atab(FILE *f,cond_p tab[NRREGTYPES][NRREGTYPES])
{
...
void
get_otab(FILE *f,cond_p tab[NRREGTYPES])
{
	int i,cnt,regtyp;
...
STATIC
void
make_instrmap(proc_p p,line_p map[])
{
...
{
	go(argc,argv,ra_initialize,ra_optimize,ra_machinit,(void (*)(char *))no_action);
	return 0;
}
...
void
print_items(item_p items[],proc_p p)
{
	int t;
	item_p item;
	interv_p iv;
