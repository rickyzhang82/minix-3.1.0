/* $Id: ra_allocl.c,v 1.3 2005/09/01 12:13:31 beng Exp $ */
/*
...
STATIC
void
count_usage(proc_p p,item_p item,short nrloops,short sloopcnt[],short dloopcnt[])
{
...
STATIC
alloc_p
cons_alloc(item_p item,interv_p timespan,short stat_usecount,
			  short dyn_usecount,lset inits,alloc_p wholeproc,
			  bool isloop,bool iswholeproc)
{
	alloc_p x;
...
STATIC
void
insert_alloc(alloc_p alloc,alloc_p *list_p)
{
...
STATIC
lset
loop_inits(loop_p lp,item_p item,bblock_p header)
{
...
STATIC
void
add_blocks(bblock_p b,cset *s,interv_p *span)
{
	Lindex pi;
...
STATIC
void
whole_lifetime(item_p item,bblock_p *ini_out,interv_p *span_out)
{
...
STATIC
lset
proc_inits(proc_p p,item_p item,bblock_p ini)
{
	lset s = Lempty_set();
...
STATIC
bool
updates_needed(loop_p lp,item_p item)
{
...
STATIC
short
countuses(lset usage,bblock_p b)
{
	short cnt = 0;
	Lindex ti;
	time_p t;
...
STATIC
void
allocs_of_item(proc_p p,item_p item,lset loops,short *sloopcnt,short *dloopcnt,alloc_p *alloc_list_p)
{
	register Lindex li;
	loop_p lp;
	bblock_p header,ini;
...
alloc_p
build_alloc_list(proc_p p,short nrloops,item_p itemlist)
{
...
void
build_rivals_graph(alloc_p alloclist)
{
