...
# ifndef NORCSID
static const_string rcsid6 = "$Id: main.c,v 1.3 2005/09/01 12:13:29 beng Exp $";
# endif
...
static
void
prn_error(int lineno,const_string fmt1,const_string fmt2,va_list ap)
{
	if (!lineno) lineno = 1;
	fprintf(stderr, fmt1, f_input, lineno);
...
/* VARARGS1 */
void
error(int lineno,const_string fmt,...)
{
	/*
	...
	va_start(ap, fmt);
	++nerrors;
	prn_error(lineno,"\"%s\", line %d: ",fmt,ap);
	va_end(ap);
}

/* VARARGS1 */
void
warning(int lineno,const_string fmt,...)
{
	/*
	 * Just a warning
	 */
	...
	va_start(ap, fmt);
	if (wflag) return;
	prn_error(lineno,"\"%s\", line %d: (Warning) ",fmt,ap);
	va_end(ap);
}

/* VARARGS1 */
void
fatal(int lineno,const_string fmt,...)
{
	/*
	 * Fatal error
	 */
...
void
install(const_string target,const_string source)
{
	/*
