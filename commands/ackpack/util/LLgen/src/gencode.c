...
# ifndef NORCSID
static const_string rcsid3 = "$Id: gencode.c,v 1.3 2005/09/01 12:13:29 beng Exp $";
#endif /* NORCSID */
...
static const_string	c_arrend =	"0 };\n";
static const_string	c_close =	"}\n";
static const_string	c_break =	"break;\n";
static const_string	c_read =	"LLread();\n";
...
STATIC void	genincrdecr(const_string,int);
...
getparams(void)
{
	 ...
	 */
	(void)getc(fact);		/* patch: some implementations of fseek
...
STATIC
void
rulecode(p,safety,mustscan,mustpop)
	register p_gram p;
	int safety, mustscan, mustpop;
{
	/*
...
alternation(pp, safety, mustscan, mustpop, lb)
	p_gram pp;
	int safety, mustscan, mustpop, lb;
{
	...
	while (g_gettype(p) != EORULE) {
		...
		if ((l->l_flag & COND) && !(l->l_flag & NOCONF)) {
			...
			if (!haddefault) {
				...
				/* unsafe = 0; -- break out of loop, so never used */
			}
			break;
		}
		p++;
...
genswhead(q, rep_kind, rep_count, safety, ispushed)
	register p_term q;
	int rep_kind, rep_count, safety, ispushed;
{
	/*
