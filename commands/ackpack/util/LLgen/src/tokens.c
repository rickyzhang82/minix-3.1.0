...
#define LL_ANSI_C 1
#define LL_LEXI scanner
/* $Id: tokens.c,v 1.3 2005/09/01 12:13:29 beng Exp $ */
#ifdef LL_DEBUG
#include <assert.h>
#include <stdio.h>
...
extern int LLnext();
extern void LLerror();
extern void LLsafeerror();
extern void LLnewlevel();
extern void LLoldlevel();
#ifndef LL_FASTER
extern void LLscan();
#endif
#ifndef LLNOFIRSTS
...
# ifndef NORCSID
static const_string	rcsidc = "$Id: tokens.c,v 1.3 2005/09/01 12:13:29 beng Exp $";
# endif
...
# endif
STATIC string	cpy(int,string,int);
STATIC const_string	vallookup(int);
STATIC void	copyact(char,char,int,int);
...
STATIC
void
copyact(char ch1,char ch2,int flag,int level)
{
	/*
...
# endif
		switch(c_class[ch]) {
		  case ISACT :
			...
			/* Be paranoid - flush output before retrieving position */
			fflush(fact);
			off = ftell(fact);
		  ...
		  case ISKEY :
			ch = input();
			do {
				/* reserved words converted to lowercase */
				if (ch >= 'A' && ch <= 'Z') {
					ch += 'a' - 'A';
				}
				*p++ = ch;
...
STATIC
string
cpy(int s,string p,int inserted)
{
	/*
