...
# ifndef NORCSID
static const_string rcsid = "$Id: compute.c,v 1.3 2005/09/01 12:13:29 beng Exp $";
# endif
...
STATIC int	first(p_set,p_gram,int);
...
do_compute(void)
{
	...
	if (subpars_sim) {
	    ...
	    for (st = start; st; st = st->ff_next) {
		setunion(start_firsts, (&nonterms[st->ff_nont])->n_first);
	    }
	}
...
STATIC
int
first(p_set setp,p_gram p,int flag)
{
	/*
