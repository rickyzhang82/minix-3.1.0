#ifndef NORCSID
static char rcsid[] = "$Id: subr.c,v 1.1 2005/09/01 12:13:28 beng Exp $";
#endif
...
int
match(token_p tp,set_p tep,int optexp)
{
	register bitno;
	token_p ct;
	result_t result;
...
void
instance(int instno,token_p token)
{
	register inst_p inp;
	int i;
	register token_p tp;
...
static
void
cinstance(int instno,token_p token,token_p tp,int regno)
{
	register inst_p inp;
	int i;
#if MAXMEMBERS != 0
...
int
eqtoken(token_p tp1,token_p tp2)
{
	register i;
...
static
int
instsize(int tinstno,token_p tp)
{
	inst_p inp;
	struct reginfo *rp;
...
#ifdef MAXSPLIT
int
split(token_p tp,int *ip,int ply,int toplevel)
{
	register c2_p cp;
...
unsigned int
docoerc(token_p tp,c3_p cp,int ply,int toplevel,int forced)
{
	unsigned cost;
...
unsigned int
stackupto(token_p limit,int ply,int toplevel)
{
...
c3_p
findcoerc(token_p tp,set_p tep)
{
	register c3_p cp;
	token_t rtoken;
	register i;
...
#ifndef NDEBUG
void
badassertion(char *asstr,char *file,int line)
{
