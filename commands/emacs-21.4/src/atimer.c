     ...
     void *client_data)
{
  ...
  if (EMACS_USECS (e_time) != 0)
    {
      EMACS_SET_USECS (e_time, 0);
      EMACS_SET_SECS (e_time, EMACS_SECS (e_time) + 1);
    }
  ...
  switch (type)
    ...
    case ATIMER_RELATIVE:
      ...
      EMACS_ADD_TIME (t->expiration, t->expiration, e_time);
      break;
  ...
  if (atimers)
    {
      ...
      EMACS_GET_TIME (now);
      EMACS_SUB_TIME (e_time, atimers->expiration, now);

#ifdef HAVE_SETITIMER
      /* Don't set the interval to 0; this disables the timer.  */
      if (EMACS_TIME_LE (atimers->expiration, now))
	{
	  EMACS_SET_SECS (e_time, 0);
	  EMACS_SET_USECS (e_time, 1000);
	}
