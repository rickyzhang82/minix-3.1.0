...
#define IN_UNEXEC
#include <config.h>
#define PERROR(file) do {printf ("Error: %s %d\n", __FILE__, __LINE__); report_error (file, new);} while (0)
#endif
...
#ifdef MINIX
      || copy_aout_text (new, a_out, a_name, new_name) < 0
#endif
...
#endif
      if (read (a_out, &ohdr, HDR_SIZE) != HDR_SIZE)
	{
	  PERROR (a_name);
	}
...
/*  hdr.a_data = bss_start - data_start;*/
  ...
  if (write (new, &hdr, HDR_SIZE) != HDR_SIZE)
    {
      PERROR (new_name);
    }
 ...
 * copy_aout_text
 *
 * Copy the text segment from the original a.out to the new a.out
 * Required since I cannot reliably determine the text segment's
 * start and end addresses. Hopefully, the text will not have been
 * changed in memory.
 */
static int copy_aout_text (new, a_out, a_name, new_name)
	int new, a_out;
	char *new_name, a_name;
{
  char page[1024];
  int n = 0;
  int total = 0;
  int todo = ohdr.a_text;

  /* Position a.out to text segment */
  if (lseek (a_out, A_TEXTPOS(ohdr), 0) < 0) 
  {
    PERROR (a_name);
  }
  ...
  while (todo > 0)
  {
    int block_size = todo < sizeof (page) ? todo : sizeof (page);
    n = read (a_out, page, block_size);
    if (n >= 0)
    {
